...
export const KatakanaSection = () => {
  const [mode, setMode] = useState<StudyMode>("chart");
  const [selectedChar, setSelectedChar] = useState<typeof KATAKANA_DATA[0] | null>(null);
  const [quizIndex, setQuizIndex] = useState(0);
  const [showAnswer, setShowAnswer] = useState(false);
  const [score, setScore] = useState({ correct: 0, wrong: 0 });
  const [shuffledData, setShuffledData] = useState<typeof KATAKANA_DATA>([]);
  ...
  const startQuiz = () => {
    const shuffled = [...KATAKANA_DATA].sort(() => Math.random() - 0.5);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold">가타카나 표</h2>
        <Button onClick={startQuiz}>퀴즈 시작</Button>
      </div>

      <div className="grid grid-cols-5 gap-2">
        {KATAKANA_DATA.map((item, index) => (
          <Card
            key={index}
            className={cn(

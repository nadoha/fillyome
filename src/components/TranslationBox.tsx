...
export const TranslationBox = memo(({
  ...
  if (isEditable) {
    return <div className="relative group animate-fade-in flex-1">
        ...
        <Textarea 
          ...
          className={`h-full min-h-[240px] resize-none text-base sm:text-lg leading-relaxed border border-border/50 bg-card/30 backdrop-blur-sm rounded-2xl p-4 pb-16 focus-visible:ring-2 focus-visible:ring-primary shadow-sm hover:shadow-md transition-all duration-200 ${isListening ? 'border-primary/60 ring-2 ring-primary/20' : ''}`} 
          autoFocus 
        />
  ...
  return <div className="relative group animate-fade-in flex-1">
      <div className="h-full min-h-[280px] border border-border/50 bg-muted/30 backdrop-blur-sm rounded-2xl p-4 pb-16 select-text transition-all duration-200 hover:bg-muted/40 hover:border-primary/30 shadow-sm" onMouseUp={onTextSelect}>
